Shader "Custom/Stylized Water V2"
{
    Properties
    {
        [Header(Densities)]
        ...
        [Header(Vertex Waves #3)]
        _Wave3Direction("Direction", Range(0, 1)) = 0
        _Wave3Amplitude("Amplitude", float) = 1
        _Wave3Wavelength("Wavelength", float) = 1
        _Wave3Speed("Speed", float) = 1
    }
        SubShader
        {
            LOD 100
            ...
            float GetWaveHeight(float3 worldPosition)
            {
                float3 dir1 = float3(cos(PI * _Wave1Direction), sin(PI * _Wave1Direction), cos(PI * _Wave1Direction));
                float3 dir2 = float3(cos(PI * _Wave2Direction), sin(PI * _Wave2Direction), cos(PI * _Wave1Direction));
                float3 dir3 = float3(cos(PI * _Wave3Direction), sin(PI * _Wave3Direction), cos(PI * _Wave1Direction));

                float wave1 = SimpleWave3D(worldPosition, dir1, _Wave1Wavelength, _Wave1Amplitude, _Wave1Speed);
                float wave2 = SimpleWave3D(worldPosition, dir2, _Wave2Wavelength, _Wave2Amplitude, _Wave2Speed);
                float wave3 = SimpleWave3D(worldPosition, dir3, _Wave3Wavelength, _Wave3Amplitude, _Wave3Speed);
            ...
            float3x3 GetWaveTBN(float3 worldPosition, float d)
            {
                float waveHeight = GetWaveHeight(worldPosition);
                float waveHeightDX = GetWaveHeight(worldPosition - float3(d, 0, 0));
                float waveHeightDZ = GetWaveHeight(worldPosition - float3(0, d, 0));
                ...
                //float3 normal = normalize(float3(waveHeight - waveHeightDY, 0, d));
            ...
            v2f vert(appdata v)
            {
                ...
                o.worldPosition.y += GetWaveHeight(o.worldPosition.xzy);
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                float3x3 tangentToWorld = GetWaveTBN(i.worldPosition.xzy, 0.1); 

                // Sample the wave normal map and calculate the world-space normal for this fragment.
                float3 normalTS = normalize(MotionFourWayChaos(_WaveNormalMap, normalize(i.worldPosition.xyz) / _WaveNormalScale, _WaveNormalSpeed, true));
                
                // nicht worldpos sonder projection?
                ...
                // If shadows are enabled, sample the shadow map.
                    ...
                    float3 reflectedColor = UNITY_SAMPLE_TEXCUBE(unity_SpecCube0, reflect(viewDirWS, UnityObjectToViewPos(i.worldPosition.xyz)));
                    ...
                    float3 sssColor = _SSSColor * GetWaveHeight(i.worldPosition.xzy);
                    ...
                    float2 foamUV = (UnityObjectToViewPos(i.worldPosition.xyz) / _FoamScale) + (_FoamNoiseScale * normalTS.xz);
                    ...
                    float3 sparkly1 = MotionFourWaySparkle(_SparklesNormalMap, UnityObjectToViewPos(i.worldPosition.xyz) / _SparkleScale, float4(1,2,3,4), _SparkleSpeed);
                    float3 sparkly2 = MotionFourWaySparkle(_SparklesNormalMap, UnityObjectToViewPos(i.worldPosition.xyz) / _SparkleScale, float4(1,0.5,2.5,2), _SparkleSpeed);
